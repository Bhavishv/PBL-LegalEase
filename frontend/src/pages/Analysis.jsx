...
import {
  ...
  PenTool, Volume2, Square, Users, MapPin
} from "lucide-react";
...
const Analysis = () => {
  ...
  useEffect(() => {
    ...
    if (showChat) {
      ...
      const wsUrl = `${API_URL.replace("http", "ws")}/ws/negotiate/${room_id}`;
      ...
      ws.onmessage = (event) => {
        ...
        if (msg.type === "CHAT_MESSAGE") {
          ...
          setUserCount(msg.data.user_count);
        }
      };
  ...
  }, [showChat, data?.filename]);

  const sendChatMessage = (text) => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      const msg = { sender: userName, text, time: new Date().toLocaleTimeString() };
  ...
  const cleanAIResponse = (text) => {
    if (!text) return "";
    return text
      .replace(/^(Understood|Absolutely|Certainly|Sure|I've analyzed|Below is).*?\n+/gi, "") // Remove starting conversational filler
      .replace(/--- ### Original Warning Clause[\s\S]*?--- ### Chain-of-Thought Analysis/gi, "") // Remove bulky COT wrappers
      .replace(/^(How to Fix It|Suggested Redline|The Catch|Negotiation Advice|Advice|Redline|Expert Remedy|Counter-Offer).*?[:\n]+/gi, "") // Strip redundant labels
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 glass-card rounded-[2.5rem] p-8 border-indigo-100 bg-gradient-to-br from-white to-indigo-50/30 relative overflow-hidden group shadow-lg">
            ...
            <h3 className="text-xl font-black text-slate-900 mb-4 flex items-center gap-2">
              <Zap className="w-5 h-5 text-indigo-500" /> The Summary
            </h3>
        ...
        {activeTab === "breakdown" ? (
                      ...
                      {selected.risk_level !== 'safe' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-slate-50">
                          <div className="p-6 bg-emerald-50 rounded-2xl print:bg-white print:border print:border-slate-100">
                            ...
                            <p className="text-xs font-bold text-emerald-900 leading-relaxed">{cleanAIResponse(selected.suggested_redline) || "No specific fix needed."}</p>
                          </div>
                          <div className="p-6 bg-amber-50 rounded-2xl print:bg-white print:border print:border-slate-100">
                            ...
                            <p className="text-xs font-bold text-amber-900 leading-relaxed">{cleanAIResponse(selected.negotiation_advice) || "Standard clause language."}</p>
                          </div>
                        </div>
                      )}
          ...
          <div className="animate-fade-in py-8 space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Radar Chart */}
              <div className="glass-card rounded-[3rem] p-10 bg-white shadow-xl border border-slate-100 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-5">
                  <Zap className="w-32 h-32 text-indigo-500" />
                </div>
                <h3 className="text-xl font-black text-slate-900 mb-8 uppercase tracking-tight flex items-center gap-2">
                  <Target className="w-5 h-5 text-indigo-500" /> Risk Distribution
                </h3>
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
                      ...
                      <Radar name="Risk" dataKey="A" stroke="#6366f1" strokeWidth={3} fill="#6366f1" fillOpacity={0.4} />
                    </RadarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-8">
                <div className="glass-card rounded-[3rem] p-10 bg-gradient-to-br from-indigo-600 via-blue-700 to-violet-800 text-white shadow-xl relative overflow-hidden group">
                  <div className="absolute -bottom-4 -right-4 p-8 opacity-20 group-hover:scale-110 transition-transform">
                    <Shield className="w-40 h-40" />
                  </div>
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200 mb-2">Industry Benchmarking</h3>
                  <div className="flex items-baseline gap-2 mb-4">
                    <span className="text-6xl font-black">{data?.dashboard_data?.industry_percentile || 85}</span>
                  ...
                  <div className="w-full h-3 bg-white/20 rounded-full mb-6 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${data?.dashboard_data?.industry_percentile || 85}%` }}
                    />
                  </div>
                ...
                <div className="glass-card rounded-[3rem] p-10 bg-white border border-slate-100 shadow-lg group hover:border-blue-200 transition-all">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center gap-2">
                    <MapPin className="w-3 h-3" /> Jurisdiction Risk
                  </h3>
                  <div className="flex items-center gap-5 mb-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transition-transform group-hover:scale-110 ${data?.jurisdiction_analysis?.is_favorable ? 'bg-emerald-100 text-emerald-600 shadow-emerald-500/10' : 'bg-rose-100 text-rose-600 shadow-rose-500/10'}`}>
                      <Gavel className="w-7 h-7" />
                    </div>
                    <div>
                      <p className="text-2xl font-black text-slate-900 leading-tight">{data?.jurisdiction_analysis?.location || "India"}</p>
                      <p className={`text-[10px] font-black uppercase tracking-widest ${data?.jurisdiction_analysis?.is_favorable ? "text-emerald-500" : "text-rose-500"}`}>
                        {data?.jurisdiction_analysis?.is_favorable ? "Low Risk Jurisdiction" : "High Risk Jurisdiction"}
                      </p>
                    </div>
                  </div>
                  <p className="text-sm font-bold text-slate-600 leading-relaxed border-l-4 border-slate-100 pl-4 mb-6">
                    {data?.jurisdiction_analysis?.description}
                  </p>
                  {data?.jurisdiction_analysis?.legal_citation && (
                    <div className="p-4 bg-slate-50 rounded-2xl text-[9px] font-black text-slate-500 uppercase tracking-widest border border-slate-100 group-hover:bg-blue-50 group-hover:border-blue-100 group-hover:text-blue-600 transition-all">
                      Legal Basis: {data.jurisdiction_analysis.legal_citation}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="glass-card rounded-[3rem] p-10 bg-white border border-slate-100 shadow-xl relative overflow-hidden">
              ...
              <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 font-serif text-slate-700 leading-relaxed whitespace-pre-wrap">
                {cleanAIResponse(data?.negotiation_letter) || "Generating your custom negotiation letter..."}
              </div>
            </div>
          </div>
        ) : (
                ...
                <div key={idx} className="bg-white rounded-[2.5rem] p-8 shadow-lg border border-slate-100 border-l-[12px] border-l-amber-500 hover:shadow-2xl transition-all">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-6 bg-slate-50 rounded-2xl">
                      ...
                      <p className="text-slate-600 font-bold text-sm leading-relaxed">{cleanAIResponse(risk.explanation)}</p>
                    </div>
                    <div className="p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
                      ...
                      <p className="text-emerald-900 font-black text-sm leading-relaxed italic">
                        "{cleanAIResponse(risk.counter_offer)}"
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-indigo-600 text-white">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className="flex items-center gap-1.5 bg-indigo-500/50 px-2 py-0.5 rounded-full">
                  <span className={`w-1.5 h-1.5 rounded-full ${isNegotiating ? 'bg-emerald-400 animate-pulse' : 'bg-slate-400'}`}></span>
                  <span className="text-[8px] font-black">{userCount} Active</span>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-slate-50">
            ...
            {roomMessages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.sender === userName ? "items-end" : "items-start"}`}>
                ...
                <div className={`p-4 rounded-2xl text-sm font-medium shadow-sm max-w-[80%] ${m.sender === userName ? "bg-indigo-600 text-white rounded-tr-none" : "bg-white text-slate-800 rounded-tl-none border border-slate-100"}`}>
                  {m.text}
                </div>
              </div>
            ))}
